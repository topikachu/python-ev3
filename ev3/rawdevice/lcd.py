...
def open_device():
    ...
    if not _initialized:
        ...
        global lcdmm
        lcdmm=mmap(fileno=_lcdfile, length=MEM_WIDTH * SCREEN_HEIGHT,flags=MAP_SHARED,prot=PROT_READ | PROT_WRITE, offset=0)
    _initialized=True
...
def draw_image(src, srcX, srcY, srcWidth, srcHegiht, destX, destY, width, height):
    ...
    i=0
    while i<height:
        _imgbuffer[destStart:destStart+width/8] = src[srcStart:srcStart+width/8]            
        srcStart+=srcWidth/8
...
def black():
    ...
    _imgbuffer=array.array('B',[0xff]*((SCREEN_WIDTH+7)/8 * SCREEN_HEIGHT))
    redraw()


def white():
    _membuffer=array.array('c',[chr(0x00)]*(MEM_WIDTH * SCREEN_HEIGHT))
    _imgbuffer=array.array('B',[0x00]*((SCREEN_WIDTH+7)/8 * SCREEN_HEIGHT))
    redraw()
